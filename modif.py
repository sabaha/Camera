...
def choose_painting(event):
    ...
    if temp == "Music":
        ...
        photo = PhotoImage(file="vermeer144.gif")
        w = photo.width()
        h = photo.height()
...
Scale(root, from_=1, to=2000, length=600, orient=VERTICAL, variable=scale, command=changeScale ).pack(side='right',padx=20)
scale.set(100)



Scale(root, from_=1, to=300, length=600, orient=VERTICAL, variable=z, command=moveInZ).pack(side='right',padx=5)
z.set(1)


Scale(root, from_=-1000, to=1000, length=600, orient=VERTICAL, variable=y, command=moveInY).pack(side='right',padx=5)


Scale(root, from_=-1000, to=1000, length=600, orient=VERTICAL, variable=x, command=moveInX).pack(side='right',padx=5)
